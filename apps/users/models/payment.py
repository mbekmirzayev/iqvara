...
class  Payment(UUIDBaseModel, SlugBaseModel, CreateBaseModel):
    ...
    payment_type = CharField(max_length=20,choices=PaymentType.choices,default=PaymentType.ONE_TIME,verbose_name=_("Payment type"),)
    installment_months = IntegerField(choices=[(3, "3 months"), (6, "6 months")], blank=True, null=True, verbose_name=_("Installment period"),)
    initial_payment_percent = DecimalField(max_digits=5,decimal_places=2,blank=True,null=True,validators=[MinValueValidator(29.0), MaxValueValidator(50.0)],default=29,verbose_name=_("Initial payment percent"),)
    card_type = CharField(max_length=10, choices=CardType.choices, verbose_name=_("Card type"))
    card_number = CharField(max_length=19,validators=[RegexValidator(r'^\d{16}$', _("Card number is invalid"))],verbose_name=_("Card number"),)
    card_expiry = CharField(max_length=5, verbose_name=_("Expiry date (MM/YY)"))
    phone_number = CharField(max_length=13,validators=[RegexValidator(r'^\+998\d{9}$', _("Phone number is invalid"))],verbose_name=_("Phone number"),)
    total_amount = DecimalField(max_digits=10, decimal_places=2, verbose_name=_("Total amount"))
    agreed_terms = BooleanField(default=False, verbose_name=_("Agreed terms"))
